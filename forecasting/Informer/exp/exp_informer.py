from data.data_loader import ETT, Solar, Weather, Wind, AUSElecDem
from exp_basic import ExpBasic
from forecasting.Informer.components.model import Informer
...
class ExpInformer(ExpBasic):
    ...
    def test(self, setting, test=0):
        ...
        # result save
        folder_path = os.path.join('output', 'Informer', 'raw', self.args.dataset, setting)
        ...
        np.save(folder_path+os.path.sep+'metrics.npy', np.array([mae, mse, rmse, mape, mspe, rse, corr]))
        np.save(folder_path+os.path.sep+'pred.npy', preds)
        np.save(folder_path+os.path.sep+'true.npy', trues)

        return

    def predict(self, setting, eb, load=False):
        self.args.eb = eb
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        batch_y = batch_y[:, -self.args.pred_len:, f_dim:f_dim+until].to(self.device)
